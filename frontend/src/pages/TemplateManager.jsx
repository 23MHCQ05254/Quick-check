              ...
              <div key={template._id || template.id} className="rounded-2xl border border-slate-900/10 bg-white/55 p-4 dark:border-white/10 dark:bg-white/[0.04]">
                ...
                <div className="mt-4 grid gap-3 sm:grid-cols-4">
                  {[
                    ...
                    ['Samples', template.trainedSamplesCount || template.extractedProfile?.metadata?.samplesUsed || template.samples?.length || 0],
                    ['Quality', template.extractedProfile?.metadata?.trainingQuality || 'n/a'],
                    ['Logos', template.extractedProfile?.logos?.length || template.extractedTemplateData?.logos?.length || 0]
                  ].map(([label, value]) => (
